...
/*function webcameqrcode(){
    ...
    function onScanSuccess(decodedText, decodedResult) {
        ...
        //console.log(`Scan result: ${decodedText}`, decodedResult);
            ...
            var latitude = $('.latitude').html();
            var dataString = 'qr_code='+decodedText+'&latitude='+latitude;
            $.ajax({
                type: "POST",
                url: "sw-mod/scan-absen-siswa/proses.php?action=absent",
                data: dataString,
                success: function (data) {
                    var results = data.split("/");
                    $results = results[0];
...
function loadData() {
    var id = $('.id').val();
    $.ajax({
        url: 'sw-mod/scan-absen-siswa/proses.php?action=data',
        type: 'POST',
        success: function(data) {
          $('.loaddata').html(data);
        }
