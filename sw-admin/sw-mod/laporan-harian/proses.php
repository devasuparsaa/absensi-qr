      ...
      echo'
        ...
        <table class="table table-bordered table-hover">
          <thead>
            ...
            echo'
            <th width="50" class="text-center bg-info">H</th>
            <th width="50" class="text-center bg-warning">I</th>
            <th width="50" class="text-center bg-danger">A</th>
            <th width="50" class="text-center bg-success">S</th>
            </tr>
          </thead>
          <tbody>';
          $limit =20; 
          ...
          while ($row = $result->fetch_assoc()){$no++;
            ...
            <tr>
              ...
              <td width="60">Masuk</td>';
               for ($d=1;$d<=$jumlahhari;$d++) {
                ...
                $query_absen ="SELECT presence_id,presence_date,time_in,present_id FROM presence WHERE $filter AND employees_id='$row[id]' ORDER BY presence_id DESC";
            ...
            $query_hadir ="SELECT presence_id FROM presence WHERE $filter AND employees_id='$row[id]' AND present_id IN ('1') ORDER BY presence_id DESC";
            $hadir= $connection->query($query_hadir);

            $query_izin="SELECT presence_id FROM presence WHERE $filter AND employees_id='$row[id]' and present_id IN ('3','4','5','7')";
            ...
            <tr>
              ...
              for ($d=1;$d<=$jumlahhari;$d++) {
                ...
                $query_absen ="SELECT presence_id,presence_date,time_in,time_out, present_id FROM presence WHERE $filter AND employees_id='$row[id]' ORDER BY presence_id DESC";
                $result_absen = $connection->query($query_absen);
                $row_absen = $result_absen->fetch_assoc();
        ...
        <p><span class="label label-info">H: Hadir</span> <span class="label label-warning">I: Izin</span> <span class="label label-danger">A: Alpha</span> <span class="label label-success">S: Sakit</span></p>
          <nav>
