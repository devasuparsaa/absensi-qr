...
if (empty($_GET['print'])) {
  ...
  header("Content-Disposition: attachment; filename=Data-Absensi-Siswa-$date.xls");
}
else {
echo'<script>
...
<html>
<head>
    <title>Cetak Data Absensi Harian Siswa</title>
    <style>
...
<body>';
      ...
      $id   = htmlentities($_GET['nis']);
      if($nis == NULL){
         $filter_employess ='';
      } 

      else{
          $filter_employess = "WHERE nis='$nis'";
      }
      echo'
      ...
      if(isset($_GET['month']) OR isset($_GET['year'])){
        ...
        <table class="customTable"
          ...
          <tbody>';
          $query ="SELECT nis,nama_siswa FROM student $filter_employess ORDER BY nis ASC";
                ...
                $row_absen = $result_absen->fetch_assoc();
                 if($row_absen['time_in']==NULL){
                  ...
                  $jam_masuk = '<b>S</b>';
                    ...
                    $jam_masuk = '<b>I</b>';
                }else if(isset($row_absen['present_id']) && $row_absen['present_id'] == '1'){
                  $jam_masuk = '<b>H</b>';
                }else{
