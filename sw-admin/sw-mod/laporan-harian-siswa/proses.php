...
case 'laporan':
      ...
      if($name==''){
        ...
        $filter_student ="WHERE student.nis='$name'";
      }
      ...
      echo'
        ...
        <table class="table table-bordered table-hover">
          ...
          $query ="SELECT nis, nama_siswa FROM student $filter_student ORDER BY nis ASC LIMIT $offset, $limit";
          ...
          while ($row = $result->fetch_assoc()){$no++;
            ...
            <tr>
              ...
              <td rowspan="2" width="150">'.$row['nama_siswa'].'</td>
              <td width="60">Masuk</td>';
               for ($d=1;$d<=$jumlahhari;$d++) {
                ...
                $query_absen ="SELECT presence_id, presence_date, time_in, present_id FROM presence_student WHERE $filter AND nis='$row[nis]' ORDER BY presence_id DESC";
                ...
                }else if(isset($row_absen['present_id']) && $row_absen['present_id'] == '2'){
                $jam_masuk = '<i class="fa fa-medkit" style="color:green;" title="Sakit"></i>';
                }else if($row_absen['time_in'] == "00:00:00"){
                $jam_masuk = '<i class="fa fa-exclamation-circle" style="color:orange;" title="Izin"></i>';
                }else{
            ...
            $query_hadir ="SELECT presence_id FROM presence_student WHERE $filter AND nis='$row[nis]' AND present_id IN ('1') ORDER BY presence_id DESC";
            $hadir= $connection->query($query_hadir);

            $query_izin="SELECT presence_id FROM presence_student WHERE $filter AND nis='$row[nis]' and present_id IN ('3','4','5','7')";
            $izin = $connection->query($query_izin);

            $query_sakit="SELECT presence_id FROM presence_student WHERE $filter AND nis='$row[nis]' and present_id='2'";
            $sakit = $connection->query($query_sakit);

            $alpha = $jumlahhari - $hadir->num_rows - $izin->num_rows - $sakit->num_rows - $sum - $libur;
            ...
            <tr>
              ...
              for ($d=1;$d<=$jumlahhari;$d++) {
                ...
                $query_absen ="SELECT presence_id, presence_date, time_in, time_out, present_id FROM presence_student WHERE $filter AND nis='$row[nis]' ORDER BY presence_id DESC";
                $result_absen = $connection->query($query_absen);
                $row_absen = $result_absen->fetch_assoc();
                if($row_absen['time_out']==NULL){
                $jam_pulang ='<span class="text-red"><i class="fa fa-times" aria-hidden="true"></i></span>';
                }else if(isset($row_absen['present_id']) && $row_absen['present_id'] == '2'){
                $jam_pulang = '<i class="fa fa-medkit" style="color:green;" title="Sakit"></i>';
                }else if($row_absen['time_out'] == "00:00:00"){
                $jam_pulang = '<i class="fa fa-exclamation-circle" style="color:orange;" title="Izin"></i>';
                }else{
        ...
        <p><span class="label label-info">H: Hadir</span> <span class="label label-success">I: Izin</span> <span class="label label-danger">A: Alpha</span> <span class="label label-warning">S: Sakit</span></p>
          <nav>
            ...
            $query = mysqli_query($connection,"SELECT COUNT(nis) AS jumData FROM student $pagination") or die ('Pagination error');
