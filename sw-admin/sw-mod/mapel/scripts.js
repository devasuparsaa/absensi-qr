$(document).ready(function() {
        ...
        if ($('input[name="nama_mapel"]').val() == '') {    
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data == 'success') {
                        swal({title: 'Berhasil!', text: 'Data Mata Pelajaran berhasil disimpan.!', icon: 'success', timer: 1500});
                        $('#modalAdd').modal('hide');
    ...
    $('.update-subject').submit(function (e) {
        ...
        if ($('#txtname').val() == '') {    
            ...
            loading();
            $.ajax({
                url: "sw-mod/mapel/proses.php?action=update",
                type: "POST",
                data: new FormData(this),
                processData: false,
                contentType: false,
    ...
    $(document).on('click', '.delete', function() { 
        ...
        swal({
            ...
            if (value) {
                loading();
                $.ajax({  
                    url: "sw-mod/mapel/proses.php?action=delete",
                    type: 'POST',    
                    data: {id: id},  
                    success: function(data) { 
                        if (data == 'success') {
