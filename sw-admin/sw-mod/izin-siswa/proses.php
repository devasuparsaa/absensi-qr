...
case 'delete':
  ...
  $nis = mysqli_real_escape_string($connection,epm_decode($_POST['nis']));
  $query_delete  ="SELECT files,permission_date,permission_date_finish from permission WHERE nis='$nis' AND permission_id='$id'";
  ...
  if($result_delete->num_rows > 0){
    ...
    while ($start <= $finish) {
          ...
          $deleted_absent  = "DELETE FROM presence_student WHERE nis='$nis' AND presence_date='$start'";
          $connection->query($deleted_absent);
    }
    $deleted  = "DELETE FROM permission_student WHERE  permission_id='$id'";
