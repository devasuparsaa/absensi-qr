...
include('../../../sw-library/sw-function.php');
    ...
    $aColumns = ['permission_id','nis','date','permission_name','permission_date','permission_date_finish','permission_description','files','type'];
    $sIndexColumn = "permission_id";
    $sTable = "permission_student";
    $gaSql['user'] = DB_USER;
    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
      extract($aRow);
        ...
        if($level_user==1){
            if(!$aRow['files']==''){
                $download ='<a href="../sw-content/izin-siswa/'.$aRow['files'].'" target="_blank" class="btn btn-sm btn-primary" title="Download">Download</a>';
            }else{
                $download ='';
            }
        $delete = '
        <button class="btn btn-sm btn-danger delete" title="Delete" data-id="'.epm_encode($aRow['permission_id']).'" data-siswa="'.epm_encode($aRow['nis']).'"><i class="fa fa-trash-o" aria-hidden="true"></i></button>';
        }
        else{
        $download ='
