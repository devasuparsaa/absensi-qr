    ...
    switch (@$_GET['action']) {
        ...
        case 'absensi-siswa':
            ...
            if (empty($_GET['nis'])) {
                ...
                $nis = mysqli_real_escape_string($connection, $_GET['nis']);
            }
            ...
            //$month = date ("m");
            $year = date("Y");
            $jumlahhari = date("t", mktime(0, 0, 0, $month, $hari, $year));
            ...
            if (empty($error)) {
                ...
                for ($d = 1; $d <= $jumlahhari; $d++) {
                    ...
                    if (date("l", mktime(0, 0, 0, $month, $d, $year)) == "Sunday" or date("l", mktime(0, 0, 0, $month, $d, $year)) == "Saturday") {
                        $warna = '#ffffff';
                        $background = '#FF0000';
                    ...
                    if (isset($_POST['month']) or isset($_POST['year'])) {
                        $month = $_POST['month'];
                        $year = $_POST['year'];
                        $filter = "nis='$nis' AND presence_date='$date_month_year' AND MONTH(presence_date)='$month' AND year(presence_date)='$year' AND nis='$nis'";
                    } else {
                        $filter = "nis='$nis' AND presence_date='$date_month_year' AND MONTH(presence_date) ='$month' AND nis='$nis'";
                    }

                    $query = "SELECT student.nis, student.nama_siswa, student.id_kelas, class.nama_kelas, class.id_jurusan, major.nama_jurusan, student.shift_id FROM student LEFT JOIN class ON student.id_kelas=class.id_kelas LEFT JOIN major ON class.id_jurusan=major.id_jurusan WHERE student.nis='$nis'";
                    ...
                    $query_absen = "SELECT presence_id, presence_date, time_in, time_out, present_id, information, TIMEDIFF(TIME(time_in), '$shift_time_in') AS selisih, IF(time_in > '$shift_time_in', 'Telat', IF(time_in = '00:00:00', 'Tidak Masuk', 'Tepat Waktu')) AS status, TIMEDIFF(TIME(time_out), '$shift_time_out') AS selisih_out FROM presence_student WHERE $filter ORDER BY presence_id DESC";
                ...
                if (isset($_POST['month']) or isset($_POST['year'])) {
                    $month = $_POST['month'];
                    $year = $_POST['year'];
                    $filter = "nis='$nis' AND MONTH(presence_date)='$month' AND year(presence_date)='$year' AND nis='$nis'";
                } else {
                    $filter = "nis='$nis' AND MONTH(presence_date) ='$month' and nis='$nis'";
                }

                $query_hadir = "SELECT presence_id FROM presence_student WHERE $filter AND present_id='1' ORDER BY presence_id DESC";
                $hadir = $connection->query($query_hadir);

                $query_sakit = "SELECT presence_id FROM presence_student WHERE $filter AND present_id='2' ORDER BY presence_id";
                $sakit = $connection->query($query_sakit);

                $query_izin = "SELECT presence_id FROM presence_student WHERE $filter AND present_id='3' ORDER BY presence_id";
                $izin = $connection->query($query_izin);


                $query_telat = "SELECT presence_id FROM presence_student WHERE $filter AND time_in>'$shift_time_in'";
                $telat = $connection->query($query_telat);

                $query_alpha = "SELECT presence_id FROM presence_student WHERE $filter AND nis='$row[nis]'";
        ...
        case 'update':
            ...
            if (empty($error)) {
                $update = "UPDATE presence_student SET present_id='$status' WHERE presence_id='$nis'";
