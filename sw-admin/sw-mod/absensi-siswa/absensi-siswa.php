...
echo'
  <div class="content-wrapper">';
    ...
    <section class="content-header">
      <h1>Laporan Data<small> Absensi Siswa</small></h1>
        <ol class="breadcrumb">
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title">Eksplor Laporan Absensi Siswa</h4>
              </div>
              ...
              <div class="form-group">
                  ...
                  <select class="form-control siswa" required>';
                    $query_siswa = "SELECT nis, nama_siswa FROM student ORDER BY nama_siswa ASC";
                    $result_siswa = $connection->query($query_siswa);
                    while ($row_siswa = $result_siswa->fetch_assoc()) {
                      echo'<option value="'.$row_siswa['nis'].'">'.$row_siswa['nama_siswa'].'</option>';
                    }
                    echo'
                  </select>
                </div>
...
<section class="content">
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      ...
      $query ="SELECT student.nis, student.nama_siswa, student.id_kelas, class.nama_kelas, class.id_jurusan, major.nama_jurusan FROM student, class, major WHERE student.id_kelas=class.id_kelas AND class.id_jurusan=major.id_jurusan AND student.nis='$nis'";
      $result = $connection->query($query);
      if($result->num_rows > 0){
          $row = $result->fetch_assoc();
        echo'
        <h4>Nama   : <span class="nama_siswa">'.$row['nama_siswa'].'</span></h4>
        <h4>Kelas : '.$row['nama_kelas'].' '.$row['nama_jurusan'].'</h4>
        <hr>
        <div class="row">
          <div class="col-md-4">
            <input type="hidden" class="id" value="'.$nis.'" readonly">
            <div class="form-group">
              <select class="form-control month" required>';
                if($month ==1){echo'<option value="01" selected>Januari</option>';}else{echo'<option value="01">Januari</option>';}
                if($month ==2){echo'<option value="02" selected>Februari</option>';}else{echo'<option value="02">Februari</option>';}
